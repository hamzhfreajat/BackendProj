...
md += '| ID | Region Name |\n|---|---|\n'
for r in data['invalid_regions'][:50]:
    md += f'| {r["id"]} | {r["name"]} |\n'
if len(data['invalid_regions']) > 50:
    md += '| ... | (Truncated for brevity) |\n'

md += '\n## ? Exact Duplicates (81 groups)\n'
md += 'These are regions with the exact same Arabic name but different IDs. You should merge the ads to one ID and delete the others.\n\n'
count = 0
for k, v in data['exact_duplicates'].items():
    ...
    md += f'- **{k}**: IDs: {", ".join(ids)}\n'
    count += 1
if len(data['exact_duplicates']) > 20:
    md += '- *(Truncated for brevity)*\n'
...
for k, v in data['normalized_duplicates'].items():
    ...
    md += f'- **{k}**: {", ".join(items)}\n'
    count += 1
if len(data['normalized_duplicates']) > 20:
    md += '- *(Truncated for brevity)*\n'

with open(r'C:\Users\hfraijat\.gemini\antigravity\brain\b5c78e29-59b6-45e8-ab63-050645670f8a\regions_report.md', 'w', encoding='utf-8') as f:
    f.write(md)
