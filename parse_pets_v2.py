...
def parse_tree(lines_text):
    ...
    for line in lines_text.strip().split('\n'):
        ...
        if depth < 6:
            ...
            gen.append(f'    ({current_l1_id}, 10, "{name}", None, "{icon}", None, None, {{"en": ["{name}"]}}),')
            l2_idx = 0; l3_idx = 0
        elif depth < 10:
            l2_idx += 1
...
generated = [
    '    (10, None, "حيوانات أليفة", "قطط، كلاب، طيور، وأسماك زينة", "pets", "#8BC34A", None, {"en": ["Pets"]}),',
    '    # ═══════════════════════════════════════════════',
    '    # PETS AND ANIMALS (parent=10)',
    '    # ═══════════════════════════════════════════════',
] + gen_list
...
for line in lines:
    ...
    if '# PETS AND ANIMALS (parent=' in line:
        skip = True
