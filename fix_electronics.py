...
generated = [
    '    # ═══════════════════════════════════════════════',
    '    # SECOND-LEVEL — إلكترونيات (parent=505)',
    '    # ═══════════════════════════════════════════════',
    '    (505, 5, "إلكترونيات", "أجهزة إلكترونية ذكية وملحقاتها", "devices", None, None, {"en": ["Electronics"]}),'
]
...
for line in lines:
    if line.startswith('├──'):
        ...
        icon = get_icon(name)
        current_l1_id = int(f"505{l1_idx:02d}")
        generated.append(f'    ({current_l1_id}, 505, "{name}", None, "{icon}", None, None, {{"en": ["{name}"]}}),')
        l2_idx = 0
...
# I will find the badly mapped 504 block and completely erase it!
# It starts mapping near '# SECOND-LEVEL — إلكترونيات (parent=504)'
new_content = re.sub(r'    # ═══════════════════════════════════════════════\n    # SECOND-LEVEL — إلكترونيات \(parent=504\).*?\]\n', '\n'.join(generated) + ']\n', content, flags=re.DOTALL)

with open('seed_categories.py', 'w', encoding='utf-8') as f:
    f.write(new_content)

print("Updated seed_categories.py replacing 504 block with 505")
